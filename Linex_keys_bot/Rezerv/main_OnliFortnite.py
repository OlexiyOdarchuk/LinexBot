...
async def process_buy_code_button(query: types.CallbackQuery):
    await query.message.delete()
    await query.message.answer("Пока что мы не поддерживаем другие игры, чтобы выбрать Fortnite нажмите кнопку ниже.", reply_markup=OtherRuIkb)

@dp.callback_query_handler(lambda query: query.data.startswith('codesru'))
async def process_buy_code_button(query: types.CallbackQuery):
    with open("img/Keys.jpg", "rb") as photo_file:
        await query.message.delete()
        uploaded_photo = await bot.send_photo(chat_id=query.message.chat.id, photo=photo_file, caption="<b>🛍️ Коды на Fortnite:</b>", reply_markup=CodesRuIkb)
...
async def process_buy_code_button(query: types.CallbackQuery):
    await query.message.delete()
    await query.message.answer("Ми поки-що не підтримуємо інші ігри, щоб выбрати Fortnite натисніть кнопку нижче.", reply_markup=OtherUaIkb)

@dp.callback_query_handler(lambda query: query.data.startswith('codesua'))
async def process_buy_code_button(query: types.CallbackQuery):
   with open("img/Keys.jpg", "rb") as photo_file:
        await query.message.delete()
        uploaded_photo = await bot.send_photo(chat_id=query.message.chat.id, photo=photo_file, caption="<b>🛍️ Коди на Fortnite:</b>", reply_markup=CodesUaIkb)
